...
(DEFMACRO new^cell (ctor &REST args)
  (LET* ((ctor ctor)
         (args args)
...
(DEFMACRO let^cell ((obj cell) &REST body)
  (LET* ((obj obj)
         (cell cell)
         (type (CICILI:INFER-TYPE cell))
    ...
    `(let ((,type ,cell-acc . ,cell)
           ((<> ,@type interior_t) ** ,cell-ptr . (FUNCTION (cast ((<> ,@type interior_t) **) ($ ,cell-acc ptr)))))
       (when (and ,cell-ptr (== (cof (cast (size_t *) ,cell-ptr)) ($ ,cell-acc adr)))
         (let (((<> ,@type interior_t) ,obj . (FUNCTION (cof (cof ,cell-ptr)))))
           ,@body)))))


(DEFMACRO letn^cell ((obj cell default) &REST body)
  (LET* ((obj obj)
         (cell cell)
         (default default)
         (type (CICILI:INFER-TYPE cell))
...
(DEFMACRO take^cell ((ptr cell) &REST body)
  (LET* ((ptr ptr)
         (cell cell)
         (type (CICILI:INFER-TYPE cell))     
         (cell-acc (GENSYM "acc_cell"))
...
(DEFMACRO taken^cell ((ptr cell default) &REST body)
  (LET* ((ptr ptr)
         (cell cell)
         (default default)
         (type (CICILI:INFER-TYPE cell))     
