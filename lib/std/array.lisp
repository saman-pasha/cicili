...
;;   (LET* ((arr arr)
;;          (type (CICILI:INFER-TYPE arr :WITH-NAME 'arr))
;;          (elem (IF (AND (= (LENGTH type) 3) (EQL (NTH 1 type) '*))
;;                    (CAR type)
;;                    (IF (AND (= (LENGTH type) 4) (EQL (CAR type) 'const) (EQL (NTH 2 type) '*))
;;                        (CADR type)
...
;;     (IF len
;;         `('(lambda* (<> new array ,elem ,(GENSYM)) (,type (const size_t len))
;;             (out (<> array ,elem))
...
(DEFMACRO nth^array (index array &KEY unchecked default)
  (LET ((index index)
    ...
    (IF unchecked
        `(nth ,index ($ ,array arr))
        `(closure ((<> nth ,@type ,(GENSYM)) :index ,index :array ,array)
           (out (<> ,@type item_t))
...
(DEFMACRO letn^array ((arr len array &REST captures) &REST body)
  (LET* ((array array)
         ...
         (type (CICILI:INFER-TYPE array))
         (arr-name (GENSYM "acc_arr"))
         (out-type (CICILI:INFER-TYPE (CAR (LAST body)))))
