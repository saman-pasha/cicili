...
static void __ciciliL_119 (const char * name , TF_Status * status ) {
  fprintf (stdout , "%s status: %s\n", name , TF_Message (status ));
}
static void __ciciliL_142 (void * data , size_t size , void * arg ) {
  fprintf (stdout , "deallocator: %f\n", (*((float *)data )));
  free (data );
}
static void __ciciliL_160 (const char * name , TF_Status * status ) {
  fprintf (stdout , "%s status: %s\n", name , TF_Message (status ));
}
int main () {
  ...
  { /* cicili#Let114 */
    ...
    __auto_type gr  = ({ /* cicili#Let117 */
                    void (*status_callback) (const char * name , TF_Status * status ) = __ciciliL_119;
          TF_Graph * gr  = TF_NewGraph ();
          TF_Operation * x  = ({ /* cicili#Let125 */
          ...
          TF_Operation * b  = ({ /* cicili#Let135 */
                TF_OperationDescription * gr_b_desc  = TF_NewOperation (gr , "Const", "gr_b_Const");
                TF_SetAttrType (gr_b_desc , "dtype", TF_FLOAT );
                { /* cicili#Block137 */
                  TF_SetAttrTensor (gr_b_desc , "value", TF_NewTensor (TF_FLOAT , ((int64_t[]){ 1}), 1, scalar_float (b_data ), sizeof(b_data), __ciciliL_142 , NULL ), status );
                  if (TF_GetCode (status ) !=  TF_OK  ) 
          ...
          TF_Operation * mul  = ({ /* cicili#Let150 */
          ...
          TF_Operation * add  = ({ /* cicili#Let155 */
