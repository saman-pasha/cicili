...
;;; Slice template macro ;;; name: Slice name ;;; element Slice element type ;;; step growth amount added to the capacity every time Slice gets full ;;; header-body custom functionalities in header file ;;; source-body custom functionalities in source file (DEFMACRO Slice-Scope (&REST body) `(LET ((header-file  (STRING-DOWNCASE (FORMAT NIL "cicili_A.h"  path name))) (guard-label  (INTERN (STRING-UPCASE (FORMAT NIL "CICILIA_H_" path name)))) (growth-step  (INTERN (STRING-UPCASE (FORMAT NIL "~A_GROWTH_STEP" name)))) (elem-type    (INTERN (STRING-DOWNCASE (FORMAT NIL "~A_elem_t" name)))) ,@body))

(DEFMACRO Slice (path name element step members-body header-body source-body) (Slice-Scope `(cicili ;; header file (header ,header-file () (guard ,guard-label
