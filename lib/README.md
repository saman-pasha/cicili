...
| [cpp/dl.cicili](cpp/dl.cicili) | `<dlfcn.h>`, `:cpp #t` | `dlopen`, `dlsym`, `dladdr` and `Dl_info`, for a C++ target that loads a sibling shared object. |
| [cpp/zigurat/zigurat.cicili](cpp/zigurat/zigurat.cicili) | ZiguratIP's `home/include`, `:cpp #t` | ZiguratIP as declarations: `stream` (std streams and `Zigurat::bufferstream`), `bigint`, `sha`, `aes`, `der`, `x509`, `tls` with the sockets under it. Each part is importable alone; cocolog's modules of the same names are the programs written against them. |
