        ...
        (cond ((or (key-eq '|struct| ty) (key-eq '|union| ty))
              ...
              ((and *function-outp* *function-spec* (key-eq 'QUOTE ty)) ; inline struct as function out type
               (let* ((fname (name *function-spec*))
                      (sname (if (str:starts-with-p "__ciciliS_" (symbol-name fname))
                                 (intern (format nil "~A_" fname))
                                 (intern (format nil "__ciciliS_~A_" fname))))
                      (struct-spec (specify-struct (append (list '|struct| sname) (cadr type)) '() :inline t)))
...
(defun specify-symbol-expr (def)
  (display "nAMe" def (specify-name< def) #\Newline)
  ...
  (cond ((key-eq  def '|nil|) (specify-nil-expr))
        ...
        (t (let* ((func (car def))
	         ...
	         (cond ((key-eq func '|code|)   (specify-code-expr (cadr def)))
		           ...
		           ((key-eq func '|include|)
                    (let ((out-spec (specify-include     def attributes)))
                      (setq attributes '())
                      out-spec))
                   ((key-eq func '|var|)
                    (let ((out-spec (specify-variable    def attributes)))
                      (setq attributes '())
                      out-spec))
		           ((key-eq func '|func|)
                    (let ((out-spec (specify-function    def attributes)))
                      (setq attributes '())
                      out-spec))
		           ((key-eq func '|method|)
                    (let ((out-spec (specify-function    def attributes)))
                      ...
                      out-spec))
		           ((key-eq func '|enum|)
                    (let ((out-spec (specify-enum        def attributes)))
                      (setq attributes '())
                      out-spec))
		           ((key-eq func '|struct|)
                    (let ((out-spec (specify-struct      def attributes)))
                      (setq attributes '())
                      out-spec))
		           ((key-eq func '|union|)
                    (let ((out-spec (specify-union       def attributes)))
                      (setq attributes '())
                      out-spec))
		           ((key-eq func '|typedef|)
                    (let ((out-spec (specify-typedef     def attributes)))
                      (setq attributes '())
                      out-spec))
		           ((key-eq func '|guard|)
                    (let ((out-spec (specify-guard       def attributes)))
                      (setq attributes '())
                      out-spec))
		           ((key-eq func '|ghost|)
                    (let ((out-spec (specify-guard       def attributes t)))
                      (setq attributes '())
                      out-spec))
		           ((key-eq func '|module|)
                    (let ((out-spec (specify-module      def attributes)))
                      (setq attributes '())
                      out-spec))                ; down here for inside macros
	        ...
	        (let ((construct (car clause)))
	          (cond ((find (char (symbol-name construct) 0) "@#")
		            ...
		            ((key-eq construct '|ghost|)
		             (add-inner (specify-guard    clause attributes t) guard-specifier)
		             (setq attributes '()))
