...
* The macro system lets you code in extremely high-order syntax that produces low-level C.
  See [builtins](builtins.cicili) and the [macro](test/macro) test folder, which contains a
  basic FastCGI web server.
* `lambda` writes an in-place function to pass as an argument or as a `defer` destructor.
  See [lambda.cicili](test/lambda/lambda.cicili).
* `defer` is a variable attribute available in `let` and `var`. It sets how a variable is
  ...
  [defer.cicili](test/lambda/defer.cicili).
...
* Inline structs can appear in a variable declaration, a function parameter, or a function
  ...
  [multi.cicili](test/multi.cicili).
* `func` in type position declares a function pointer.
* See [basic.cicili](test/basic.cicili) for struct samples and
  [control.cicili](test/control.cicili) for control structures.
...
| `closure` / `closure*` / `def-closure` | a function that captures its environment | [Closures](#closures) |
...
| `resolve` | `func` | `(resolve #f)` opts the function out of the resolver |
...
POSIX so the resolver can infer their types — see [lib/std/c/README.md](lib/std/c/README.md).
...
without running the resolver and are never handed to the C compiler; `source` targets are
...
…
{
  __auto_type inc = __ciciliL_1042;
  printf ("%d\n", inc (41));
}
```
...
| `--verbose` | add `-v` to the `gcc` / `libtool` commands — useful when linking many libraries |
...
| `--separate` | write each resolver run to its own `.run#.c` file; useful for debugging the resolver |
...
* [test/basic.cicili](test/basic.cicili) — structs, unions, members, designated initializers
* [test/control.cicili](test/control.cicili) — every control structure in one file
* [test/var.cicili](test/var.cicili) — every variable and initializer form
* [test/multi.cicili](test/multi.cicili) — multi-value returns
* [test/lambda](test/lambda) — lambda, defer, closures
* [test/mylib.cicili](test/mylib.cicili) — a header, three objects, a library and an executable
* [builtins.cicili](builtins.cicili) — the macro layer, and the best source of idiomatic Cicili
* [lib/std/c/README.md](lib/std/c/README.md) — C standard library and POSIX declarations
* [README.md](README.md) — the functional layer: ADTs, pattern matching, Functors, Monads
