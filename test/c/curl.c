  ...
  ({ /* progn169 */
    ({ /* letn171 */
      ...
      { /* let173 */
        cu_buf body  = { NULL , 0};
        // ----------
        check ("get a file:// url", ((long long)({ /* progn178 */
            ({ /* letn180 */
              int cu_rc_176  = 0;
              // ----------
              cu_rc_176  = ({ /* progn182 */
                    ({ /* letn184 */
                      int cu_rc_177  = 0;
                      // ----------
                      if (cu_rc_177  ==  CURLE_OK  )
                        cu_rc_177  = curl_easy_setopt (h , CURLOPT_URL , ((const char *)URL ));
                      if (cu_rc_177  ==  CURLE_OK  )
                        cu_rc_177  = ({ /* progn190 */
                      ...
                      if (cu_rc_177  ==  CURLE_OK  )
                        cu_rc_177  = curl_easy_setopt (h , CURLOPT_TIMEOUT , ((long)10));
                      cu_rc_177 ;
                    });
                  });
              if (cu_rc_176  ==  CURLE_OK  )
                cu_rc_176  = curl_easy_perform (h );
              cu_rc_176 ;
            });
          })), 0);
        ...
        check ("get again, appending", ((long long)({ /* progn202 */
            ({ /* letn204 */
              int cu_rc_200  = 0;
              // ----------
              cu_rc_200  = ({ /* progn206 */
                    ({ /* letn208 */
                      int cu_rc_201  = 0;
                      // ----------
                      if (cu_rc_201  ==  CURLE_OK  )
                        cu_rc_201  = curl_easy_setopt (h , CURLOPT_URL , ((const char *)URL ));
                      if (cu_rc_201  ==  CURLE_OK  )
                        cu_rc_201  = ({ /* progn214 */
                              curl_easy_setopt (h , CURLOPT_WRITEFUNCTION , cu_write );
                              curl_easy_setopt (h , CURLOPT_WRITEDATA , (&body ));
        ...
        check ("reused after free", ((long long)({ /* progn222 */
            ({ /* letn224 */
              int cu_rc_220  = 0;
              // ----------
              cu_rc_220  = ({ /* progn226 */
                    ({ /* letn228 */
                      int cu_rc_221  = 0;
                      // ----------
                      if (cu_rc_221  ==  CURLE_OK  )
                        cu_rc_221  = curl_easy_setopt (h , CURLOPT_URL , ((const char *)URL ));
                      if (cu_rc_221  ==  CURLE_OK  )
                        cu_rc_221  = ({ /* progn234 */
                              curl_easy_setopt (h , CURLOPT_WRITEFUNCTION , cu_write );
                              curl_easy_setopt (h , CURLOPT_WRITEDATA , (&body ));
      ...
      { /* let240 */
        cu_buf empty  = { NULL , 0};
        // ----------
        check ("a missing file is not CURLE_OK", ((long long)(CURLE_OK  !=  ({ /* progn245 */
              ({ /* letn247 */
                int cu_rc_243  = 0;
                // ----------
                cu_rc_243  = ({ /* progn249 */
                      ({ /* letn251 */
                        ...
                        if (cu_rc_244  ==  CURLE_OK  )
                          cu_rc_244  = curl_easy_setopt (h , CURLOPT_URL , ((const char *)"file:///no/such/cicili/file"));
                        if (cu_rc_244  ==  CURLE_OK  )
                          cu_rc_244  = ({ /* progn257 */
                                curl_easy_setopt (h , CURLOPT_WRITEFUNCTION , cu_write );
                ...
                if (cu_rc_243  ==  CURLE_OK  )
                  cu_rc_243  = curl_easy_perform (h );
                cu_rc_243 ;
              });
            }) )), 1);
        cu_buf_free ((&empty ));
        check ("an unknown scheme is UNSUPPORTED_PROTOCOL", ((long long)({ /* progn265 */
            ({ /* letn267 */
              int cu_rc_263  = 0;
              // ----------
              cu_rc_263  = ({ /* progn269 */
                    ({ /* letn271 */
                      int cu_rc_264  = 0;
                      // ----------
                      if (cu_rc_264  ==  CURLE_OK  )
                        cu_rc_264  = curl_easy_setopt (h , CURLOPT_URL , ((const char *)"nosuchscheme://host/path"));
                      if (cu_rc_264  ==  CURLE_OK  )
                        cu_rc_264  = ({ /* progn277 */
                              curl_easy_setopt (h , CURLOPT_WRITEFUNCTION , cu_write );
                              curl_easy_setopt (h , CURLOPT_WRITEDATA , (&empty ));
      ...
      { /* let283 */
        cu_buf after  = { NULL , 0};
        // ----------
        check ("still usable after reset", ((long long)({ /* progn288 */
            ({ /* letn290 */
              int cu_rc_286  = 0;
              // ----------
              cu_rc_286  = ({ /* progn292 */
                    ({ /* letn294 */
                      int cu_rc_287  = 0;
                      // ----------
                      if (cu_rc_287  ==  CURLE_OK  )
                        cu_rc_287  = curl_easy_setopt (h , CURLOPT_URL , ((const char *)URL ));
                      if (cu_rc_287  ==  CURLE_OK  )
                        cu_rc_287  = ({ /* progn300 */
                              curl_easy_setopt (h , CURLOPT_WRITEFUNCTION , cu_write );
      ...
      { /* let305 */
        ...
        ({ /* progn309 */
          ({ /* letn311 */
            struct curl_slist * cu_hs_308  = NULL ;
            // ----------
            cu_hs_308  = curl_slist_append (hs , "Accept: application/json");
            if (cu_hs_308  !=  NULL  )
              hs  = cu_hs_308 ;
            cu_hs_308  = curl_slist_append (hs , "X-Cicili: 1");
            if (cu_hs_308  !=  NULL  )
              hs  = cu_hs_308 ;
            hs ;
          });
        ...
        check ("setting it is accepted", ((long long)({ /* progn319 */
            ({ /* letn321 */
              int cu_rc_318  = 0;
              // ----------
              if (cu_rc_318  ==  CURLE_OK  )
                cu_rc_318  = curl_easy_setopt (h , CURLOPT_HTTPHEADER , hs );
              cu_rc_318 ;
            });
          })), ((long long)CURLE_OK ));
        ({ /* progn326 */
