...
(source "concepts.c" (:std #t :compile #t :link "-L{$CCL} -lhaskell.o -L{$CWD} concepts.o -o main")
        ...
        (main
          ...
          (io (get^Semigroup^Concat^List^int)
            (_ mappend
               (letin ((l1 ((<> new List int) '{ 1 3 5 7 }) free^List^int)
                       (l2 ((<> new List int) '{ 2 4 6 8 }) free^List^int))
                 (format #t "Concat of Lists (mappend Semigroup) of '{ 1 3 5 7 } and '{ 2 4 6 8 } is:\n")
                 (show^List^int (mappend l1 l2)))))
