...
(defun compile-generic (spec lvl globals &key ((:nested is-nested) nil))
  (let ((name (name spec))
    ...
    (if (null *specific*)
        (progn
          ; nothing without specific clause
          )
        (progn
...
(defun specify-specific (def attrs)
  (when (> (length attrs) 0) (error (format nil "wrong attributes ~A" attrs)))
  (let* ((args (nth 1 def))
	     (clauses (nthcdr 2 def))
	     (specific-specifier (make-specifier (gensym "ciciliSpecific") '|@SPECIFIC| nil nil nil nil nil nil nil)))
    (setf (default specific-specifier) args)
    (let ((attributes '())
      ...
      (dolist (clause clauses)
	    (if (consp clause)
	        (let ((construct (car clause)))
	          (cond ((find (char (symbol-name construct) 0) "@#")
		             (add-inner (specify-preprocessor clause attributes) specific-specifier)
		             (setq attributes '()))
		            ((key-eq construct '|static|)   (push clause attributes))
		            ((key-eq construct '|decl|)     (push clause attributes))
		            ((key-eq construct '|inline|)   (push clause attributes))
		            ...
		            ((key-eq construct '|defer|)    (push clause attributes))
		            ((key-eq construct '|func|)
		             (add-inner (specify-function clause attributes) specific-specifier) (setq attributes '()))
		            ((key-eq construct '|method|)
		             (add-inner (specify-function clause attributes) specific-specifier) (setq attributes '()))
		            ((key-eq construct '|struct|)
		             (add-inner (specify-struct   clause attributes) specific-specifier) (setq attributes '()))
		            ((key-eq construct '|union|)
		             (add-inner (specify-union    clause attributes) specific-specifier) (setq attributes '()))
		            ((key-eq construct '|guard|)
		             (add-inner (specify-guard    clause attributes) specific-specifier) (setq attributes '()))
		            ((key-eq construct '|ghost|)
		             (add-inner (specify-guard    clause attributes t) specific-specifier) (setq attributes '()))
		            ((key-eq construct '|module|)
		             (add-inner (specify-module   clause attributes) specific-specifier) (setq attributes '()))
		            ((key-eq construct '|generic|)
                     (when *specific*
                       (progn
                         
                         ))
		             (add-inner (specify-generic  clause attributes) specific-specifier) (setq attributes '()))
		            (t (add-inner (specify-expr   clause) specific-specifier))))
	        (error (format nil "syntax error ~A" clause))))
      (setf *specific* tmp-specific)
      specific-specifier)))

(defun compile-specific (spec lvl globals &key ((:nested is-nested) nil))
  (let ((name (name spec))
        (args (default spec)))
    (set-ast-line (output "~&#define ~A" name))
    (output "(")
    (loop for arg in args
          ...
          for i from 0 to l
          do (progn
               (set-ast-line (output "~A" arg))
               (when (< i l) (output ","))))
    (output ") \\~%")
    (let ((tmp-specific *specific*)) 
      (setf *specific* t)
      (maphash #'(lambda (in-name in-spec)
		           (case (construct in-spec)
		             ('|@VAR|      (compile-variable     in-spec lvl globals))
		             ...
		             ('|@MODULE|   (compile-module       in-spec lvl globals))
		             ('|@GENERIC|  (compile-generic      in-spec lvl globals :nested t))
		             ('|@specific| (compile-specific     in-spec lvl globals :nested t))
		             (otherwise    (compile-form         in-spec lvl globals)
                                   (output "~%"))))
	           (inners spec))
      (setf *specific* tmp-specific))
