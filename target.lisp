	    ...
	    (if (consp clause)
	        (let ((construct (car clause)))
	          (cond ((find (char (symbol-name construct) 0) "@#")
		            ...
		            ;;  (add-inner (specify-generic  clause attributes) target-specifier) (setq attributes '()))
		            ;; ((key-eq construct '|specific|)
		            ;;  (add-inner (specify-specific clause attributes) target-specifier) (setq attributes '()))
	         ...
	         (progn
	           ...
	           (maphash #'(lambda (in-name in-spec)
		                    (case (construct in-spec)
			                  ...
			                  ('|@VAR|      (compile-variable     in-spec 0 globals t))
			                  ...
			                  ('|@GHOST|    (compile-guard        in-spec 0 globals t))
			                  ('|@MODULE|   (compile-module       in-spec 0 globals))
			                  ('|@GENERIC|  (compile-generic      in-spec 0 globals))
