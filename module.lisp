        ...
        (progn ; in header file
          (maphash #'(lambda (in-name in-spec)
		               (case (construct in-spec)
		                 ('|@VAR|      (compile-variable     in-spec lvl globals spec t :unique t))
		                 ('|@FUNC|     (compile-function     in-spec lvl globals spec :unique t))
		                 ('|@METHOD|   (compile-function     in-spec lvl globals spec :unique t))
		                 ('|@PREPROC|  (compile-preprocessor in-spec lvl globals spec))
		                 ('|@ENUM|     (compile-enum         in-spec lvl globals spec :unique t))
		                 ('|@STRUCT|   (compile-struct       in-spec lvl globals spec :unique t))
		                 ('|@UNION|    (compile-union        in-spec lvl globals spec :unique t))
