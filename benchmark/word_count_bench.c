#include <stdio.h>
#include <stddef.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>
#include "../haskell.h"
#include <errno.h>
#include <time.h>
typedef FILE * cfile_t ;
#ifndef __Either_String_cfile_t__H_DECL__
#define __Either_String_cfile_t__H_DECL__
#ifndef __H___h_Either_ctor_t__
#define __H___h_Either_ctor_t__
typedef enum __h_Either_ctor_t {
  __h_Right_t = 0,
  __h_Left_t = 1
} __h_Either_ctor_t;
#endif /* __H___h_Either_ctor_t__ */ 
typedef struct Either_String_cfile_t Either_String_cfile_t ;
typedef void (*free_Either_String_cfile_t_t) (Either_String_cfile_t * this );
typedef struct Either_String_cfile_t__H_Table {
  free_Either_String_cfile_t_t freeData ;
} Either_String_cfile_t__H_Table;
typedef struct Either_String_cfile_t {
  const Either_String_cfile_t__H_Table * __h_table ;
  char __h_ctor ;
  union { /* ciciliUnion114 */
    struct { /* ciciliStruct115 */
      String __h_0_mem ;
    } Left , _0 ;
    struct { /* ciciliStruct116 */
      cfile_t __h_0_mem ;
    } Right , _ ;
  } __h_data ;
} Either_String_cfile_t;
Either_String_cfile_t Left_String_cfile_t (String error );
Either_String_cfile_t Right_String_cfile_t (cfile_t value );
__attribute__((weak)) Either_String_cfile_t Default_Either_String_cfile_t (cfile_t value ) {
  return Right_String_cfile_t (value );
}
const Either_String_cfile_t__H_Table * const get_Either_String_cfile_t__H_Table ();
void free_Either_String_cfile_t (Either_String_cfile_t * this );
#endif /* __Either_String_cfile_t__H_DECL__ */ 
#ifndef __Either_String_cfile_t__H_IMPL__
#define __Either_String_cfile_t__H_IMPL__
void free_Either_String_cfile_t (Either_String_cfile_t * this ) {
}
const Either_String_cfile_t__H_Table * const get_Either_String_cfile_t__H_Table () {
  static const Either_String_cfile_t__H_Table table  = { free_Either_String_cfile_t };
  return (&table );
}
Either_String_cfile_t Left_String_cfile_t (String error ) {
  return ((Either_String_cfile_t){ get_Either_String_cfile_t__H_Table (), __h_Left_t , .__h_data.Left = { error }});
}
Either_String_cfile_t Right_String_cfile_t (cfile_t value ) {
  return ((Either_String_cfile_t){ get_Either_String_cfile_t__H_Table (), __h___t , .__h_data._ = { value }});
}
#endif /* __Either_String_cfile_t__H_IMPL__ */ 
Either_String_cfile_t writeTmpFile () {
  { /* cicili#Let149 */
    FILE * tmpf  = tmpfile ();
    // ----------
    if (tmpf  ==  NULL  )
      { /* cicili#Block153 */
        { /* cicili#Let155 */
          __auto_type err  = strerror (errno );
          // ----------
          return Left_String_cfile_t (pure_String (err , strlen (err )));
        }
      }
    fputs ("Alan Turing\n", tmpf );
    fputs ("John von Neumann\n", tmpf );
    fputs ("Alonzo Church\n", tmpf );
    return Right_String_cfile_t (tmpf );
  }
}
List_String safeReadFile (FILE * file ) {
  return (((file  ==  NULL  )) ? Nil_String () : ({ /* cicili#Let161 */
        char buf [8];
        size_t count  = 0;
        // ----------
        count  = fread (buf , sizeof(char), sizeof(buf), file );
        (((count  ==  0 )) ? Nil_String () : (((count  <  sizeof(buf) )) ? Cons_String (pure_String (buf , count ), Nil_String ()) : Cons_String (pure_String (buf , count ), safeReadFile (file ))));
      }));
}
int count_words (String text ) {
  return ({ /* cicili#Let173 */
      typeof((((text -> __h_data ). Cons ). __h_0_mem )) ch ;
      typeof((((text -> __h_data ). Cons ). __h_1_mem )) tail ;
      // ----------
      
      ({ /* cicili#Let177 */
        bool __h_case_result  = (true  &&  (((text -> __h_ctor ) ==  __h_Cons_t  ) &&  ((({ /* cicili#Progn178 */
                  (ch  =  (((text -> __h_data ). Cons ). __h_0_mem ) );
                  true ;
                }) &&  ({ /* cicili#Progn180 */
                  (tail  =  (((text -> __h_data ). Cons ). __h_1_mem ) );
                  true ;
                }) ) &&  (true  &&  ((tail -> __h_ctor ) ==  __h_Cons_t  ) ) ) ) );
        // ----------
        ((__h_case_result ) ? ({ /* cicili#Progn183 */
            ((((ch  ==  ' ' ) ||  (ch  ==  '\n' ) )) ? (1 +  count_words (tail ) ) : count_words (tail ));
          }) : ({ /* cicili#Let188 */
            // ----------
            
            ({ /* cicili#Progn192 */
              1;
            });
          }));
      });
    });
}
void iter_words (List_String list ) {
  { /* cicili#Let197 */
    typeof((((list -> __h_data ). Cons ). __h_0_mem )) str ;
    typeof((((list -> __h_data ). Cons ). __h_1_mem )) tail ;
    // ----------
    
    ({ /* cicili#Let201 */
      bool __h_case_result  = (true  &&  (((list -> __h_ctor ) ==  __h_Cons_t  ) &&  (({ /* cicili#Progn202 */
              (str  =  (((list -> __h_data ). Cons ). __h_0_mem ) );
              true ;
            }) &&  ({ /* cicili#Progn204 */
              (tail  =  (((list -> __h_data ). Cons ). __h_1_mem ) );
              true ;
            }) ) ) );
      // ----------
      if (__h_case_result )
        { /* cicili#Block208 */
          { /* cicili#Block210 */
            ({ /* cicili#Progn213 */
              show_String (str );
              printf (" Word count: %d\n", count_words (str ));
            });
            iter_words (tail );
            free_String ((&str ));
          }
        }
    });
  }
}
void file_close (FILE ** file_ptr ) {
  ({ /* cicili#Progn217 */
    printf ("file closed deferred\n");
  });
  fclose ((*file_ptr ));
}
int main () {
  { /* cicili#Let221 */
    __auto_type start  = time (NULL );
    // ----------
    for (int i  = 0; (i  <  1000000 ); (++i )) {
        ({ /* cicili#Let226 */
          __auto_type tmpf  = writeTmpFile ();
          // ----------
          { /* cicili#Let230 */
            typeof((((tmpf . __h_data ). Left ). __h_0_mem )) error ;
            // ----------
            
            ({ /* cicili#Let234 */
              bool __h_case_result  = (true  &&  (((tmpf . __h_ctor ) ==  __h_Left_t  ) &&  ({ /* cicili#Progn235 */
                    (error  =  (((tmpf . __h_data ). Left ). __h_0_mem ) );
                    true ;
                  }) ) );
              // ----------
              if (__h_case_result )
                { /* cicili#Block239 */
                  ({ /* cicili#Let242 */
                    __auto_type error  __attribute__((__cleanup__(free_String ))) = error ;
                    // ----------
                    ({ /* cicili#Let245 */
                      __auto_type tmp_stdout  = stdout ;
                      // ----------
                      stdout  = stderr ;
                      printf ("File opening error: ");
                      show_String (error );
                      putchar ('\n');
                      stdout  = tmp_stdout ;
                    });
                  });
                }
              else
                { /* cicili#Let250 */
                  typeof((((tmpf . __h_data ). Right ). __h_0_mem )) file ;
                  // ----------
                  
                  ({ /* cicili#Let254 */
                    bool __h_case_result  = (true  &&  (((tmpf . __h_ctor ) ==  __h_Right_t  ) &&  ({ /* cicili#Progn255 */
                          (file  =  (((tmpf . __h_data ). Right ). __h_0_mem ) );
                          true ;
                        }) ) );
                    // ----------
                    if (__h_case_result )
                      { /* cicili#Block259 */
                        ({ /* cicili#Let262 */
                          __auto_type file  __attribute__((__cleanup__(file_close ))) = file ;
                          // ----------
                          rewind (file );
                          ({ /* cicili#Let265 */
                            __auto_type words  __attribute__((__cleanup__(__h_free_class_router ))) = ({ /* cicili#Let267 */
                              __auto_type tmp_obj  = safeReadFile (file );
                              // ----------
                              ((void)(tmp_obj -> __h_table -> freeClass ));
                              tmp_obj ;
                            });
                            // ----------
                            { /* cicili#Let272 */
                              // ----------
                              
                              ({ /* cicili#Let276 */
                                bool __h_case_result  = (true  &&  ((words -> __h_ctor ) ==  __h_Cons_t  ) );
                                // ----------
                                if (__h_case_result )
                                  { /* cicili#Block279 */
                                    { /* cicili#Block281 */
                                      ({ /* cicili#Progn284 */
                                        printf ("File loaded successfully!\n");
                                      });
                                      iter_words (words );
                                    }
                                  }
                                else
                                  { /* cicili#Let289 */
                                    // ----------
                                    
                                    ({ /* cicili#Let293 */
                                      bool __h_case_result  = (true  &&  ((words -> __h_ctor ) ==  __h_Nil_t  ) );
                                      // ----------
                                      if (__h_case_result )
                                        { /* cicili#Block296 */
                                          ({ /* cicili#Let299 */
                                            __auto_type tmp_stdout  = stdout ;
                                            // ----------
                                            stdout  = stderr ;
                                            printf ("Error: nothing to read\n");
                                            stdout  = tmp_stdout ;
                                          });
                                        }
                                    });
                                  }
                              });
                            }
                          });
                        });
                      }
                  });
                }
            });
          }
        });
    }
    fprintf (stdout , "%lu seconds elapsed\n", ((unsigned long)(time (NULL ) -  start  )));
  }
}
