...
(defun infer-type-spec (id spec &key (with-name nil with-name-p))
  (format t "INFER SPEC REQUESTED: ~A~%  ~A~%" id spec)
...
(defun infer-type (id &key (with-name nil with-name-p))
  ...
  (let* ((id (expand-macros id))
         (spec (cond ((numberp id) (return-from infer-type (if with-name
                                                               (list '|const| '|int| with-name)
                                                               (list '|const| '|int|))))
                     ...
                     ((listp id) (let* ((clause (expand-macros id))
                                   ...
                                   (cond ((key-eq func '|cast|)
                                         ...
                                         ((key-eq func '|letn|)
                                          ...
                                          (return-from infer-type (infer-type (expand-macros (car (last clause))) :with-name with-name)))
                                         ((key-eq func '|progn|)
                                          (format t "RRRRR2 ~A~%" (expand-macros (car (last clause))))
                                          (return-from infer-type (infer-type (expand-macros (car (last clause))) :with-name with-name)))
                                         ((key-eq func '|aof|)
                                          (format t "RRRRR3 ~A~%" (expand-macros (cdr clause)))
                                          (return-from infer-type
                                            (let ((ty (infer-type (expand-macros (cdr clause)) :with-name with-name)))
                                              (multiple-value-bind (const typeof modifier const-ptr variable array)
		                                          (specify-type< ty)
                                                (remove nil (list const typeof
                                                                  (cond
                                                                    ((key-eq '|*|  modifier) '|**|)
                                                                    ((key-eq '|**| modifier) '|***|)
                                                                    (t '|*|))
                                         ...
                                         ((and (= (length clause) 2) (find func *unaries* :test #'key-eq))
                                          (return-from infer-type (infer-type (cadr clause) :with-name with-name)))
                                         ((and (> (length clause) 2) (find func *operators* :test #'key-eq))
                                          (return-from infer-type (infer-type (cadr clause) :with-name with-name)))
                                         ((key-eq func '$) (let ((struct (infer-type (cadr clause))))
                                                             (format t "SSSSSS ~A~%" struct)))
                                         ((key-eq func '|return|) (infer-type (cadr clause) :with-name with-name))
                                         ((key-eq func '?) (infer-type (cadr clause) :with-name with-name))
                                         ((symbolp func) (*gets* func))
                                         ) ; cond
                                   ))
                     (t (deep-typeof id)))))
    (if spec
        (if with-name-p (infer-type-spec id spec :with-name with-name) (infer-type-spec id spec))
