                     ...
                     ((listp id) (let* ((clause (expand-macros id))
                                   ...
                                   (cond ((key-eq func '|cast|)
                                         ...
                                         ((key-eq func '|letn|)
                                          ...
                                          (return-from infer-type (infer-type (expand-macros (car (last clause)))
                                                                    :with-name with-name :copy-name copy-name)))
                                         ((key-eq func '|progn|)
                                         ...
                                         ((key-eq func '|aof|)
                                          ...
                                          (return-from infer-type
                                            (let ((ty (infer-type (expand-macros (cdr clause))
                                                        :with-name with-name :copy-name copy-name)))
                                         ...
                                         ((key-eq func '|return|) (infer-type (cadr clause)
                                                                    :with-name with-name :copy-name copy-name))
                                         ((key-eq func '?) (infer-type (cadr clause)
                                                             :with-name with-name :copy-name copy-name))
