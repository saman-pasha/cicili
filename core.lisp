...
(defvar *type-inference-time* nil)
          ...
          (cond ((eql const-val '|@ATOM|)
                ...
                ((eql const-val '|@UNARY|)
                 (let ((ty (deep-typeof id (default spec))))
                   (cond ((eql (name spec) '&)
                          (let ((new_spec (specify-variable (append (list '|var|) (infer-type (list '|aof| (name ty)))) '())))
                            (setf (name new_spec) (name ty))
                            (setf (is-moved new_spec) (is-moved ty))
                            new_spec))
                         ((eql (name spec) '*)
                          (let ((new_spec (specify-variable (append (list '|var|) (infer-type (list '|cof| (name ty)))) '())))
                            (setf (name new_spec) (name ty))
                            (setf (is-moved new_spec) (is-moved ty))
                            new_spec))
                         (t ty))))
         ...
         (spec (cond ((numberp id) (return-from infer-type
                     ...
                     ((listp id) (let* ((clause (expand-macros id))
                                   ...
                                   (cond ((key-eq func '|cast|)
                                                ...
                                                (if (key-eq (car func) '|lambda*|)
                                                    (let ((out (nth 3 func)))
                                                      (if (key-eq (car out) '|out|)
                                                          ...
                                                          (list '|void|)))))))
                                         ((key-eq func '|letn|)
                                          (setq *type-inference-time* t)
                                          (let ((letn-spec (specify-let clause)))
                                            (setq *type-inference-time* nil)
                                            (*push* (name letn-spec))
                                         ...
                                         ((key-eq func '|progn|)
                                          (setq *type-inference-time* t)
                                          (let ((progn-spec (specify-progn clause)))
                                            (setq *type-inference-time* nil)
