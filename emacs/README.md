...
`authority backend body cicili compiler config core mode module package resolver
specifier target`
...
Two things fail in `draft/` and did so before the rename — confirmed by diffing
against the git blobs, not introduced here:

- 5 dangling imports in `draft/in-progress/TensorFlow` (`../Slice/slice`,
  `cicili_graph`, `cicili_session`, `cicili_dataset`) point at files that never existed.
- 8 files do not balance their parens: `draft/in-progress/TensorFlow/{optimizer,model,
  conv2d,rnn,dense,sample_pure}`, `draft/in-progress/DB/postgres`, `test/macro/sample`.
